...
use crate::utility::interface::utils::{App, InputMode};
...
pub(crate) fn user_input() -> Result<(), Box<dyn Error>> {
    // setup terminal
    enable_raw_mode()?;
...
fn ui(f: &mut Frame, app: &App) {
    ...
    let messages: Vec<ListItem> = app
        ...
        .enumerate()
        .map(|(i, m)| {
            let content = Line::from(Span::raw(format!("{}: {m}", app.messages.len() - i - 1)));
