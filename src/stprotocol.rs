...
use bstr::ByteSlice;
use std::{env, fs::File, io::{Read, Write}, net::TcpStream, thread, u8, time::Duration, net::Shutdown};
...
impl HandleSession {
    ...
    pub fn HandleConnectionLoss(&self) {
        ...
        loop {
            ...
            if let Ok(mut socket) = TcpStream::connect(self.socket.local_addr().unwrap().ip().to_string()) {
                match socket
...
impl Client for HandleSession {
    ...
    fn receiving(&mut self, buffer: StructStone) -> StructStone {
        ...
        if buffer_size != 12 {
            ...
            self.packet = StructStone::from(buffer.get_header(), payload);
    ...
    fn download(&mut self) -> Result<(), ()> {
        let path = match String::from_utf8(self.packet.get_file()) {
            Ok(ok) => ok,
            Err(_) => {
                return self.send_msg("File Not Found")
            }
        };
